# Tracked Jobs & Active Notifications

This file is the canonical source of truth for all PSU and government job vacancies Avanish has applied for, plans to apply for, or is actively tracking. 

---

## 📅 Mandatory Pre-Session Checking Rule
Before answering any user query, the agent must check this file, compare it with the current date, and perform a rapid web search or lookups to ensure there are no new notifications (such as Admit Cards released, Results declared, or Date changes) for these specific jobs.

---

## 🚀 Category I: Staged & Completed Tracks (Awaiting Results/Next Rounds)

These are jobs where Avanish has already completed significant selection rounds. We are actively monitoring them for results or next round schedules.

### 1. MSTC -- Management Trainee (Systems)
* **Status**: **Not Selected** (Final merit list published July 2026).
* **Next Event**: None (closed).
* **Website**: `mstcindia.co.in/careers`

### 2. STPI -- MTS E-I (Scientist B)
* **Status**: **Not Selected** (Final Result: 68.80 / 100).
* **Score Breakdown**: CBT 96/150 (64% = 44.80/70) + Interview **80.00/100** (24.00/30) = 68.80.
* **Website**: `stpi.in`

### 3. STPI -- MTSS ES-IV (Member Technical StaffS)
* **Status**: **CBT Completed on 31 May 2026** (150/150 Qs attempted, no negative marking). Awaiting results.
* **Conductor**: **NIELIT**.
* **Website**: `stpi.in` / `nielit.gov.in`

### 4. UCO Bank -- Software Developer in JMGS-I
* **Status**: **Selected** (Appointment Letter received 08.07.2026). Joining in September 2026 (tentative).
* **Next Event**: Reporting/Joining (September 2026 tentative).
* **Website**: `ucobank.com/careers`

### 5. Currency Note Press (CNP) Nashik -- Supervisor (IT) Level-S1
* **Status**: **Application Submitted on 03 June 2026**.
* **Application Details**: Reg No: `CNP26094296` | Pass: `Anchal@@11`
* **Next Event**: **CBT TENTATIVELY SCHEDULED FOR 3rd/4th WEEK OF AUGUST 2026** (Official Notice No. CNPN/HR/CR-2026/01/05 dated 23.07.2026). Exact date/time/admit card release pending on `cnpnashik.spmcil.com`.
* **Website**: `cnpnashik.spmcil.com`

### 6. BSNL JTO (Junior Telecom Officer) Telecom -- Open Market Direct Recruitment
* **Status**: **CONFIRMED CBT EXAM DATE: AUGUST 23, 2026 (SUNDAY)** 🔥
* **Notification Ref**: `BSNLCO-11/12(11)/1/2026-RECTT-CO` dated 01.08.2026.
* **Admit Card Date**: **19 August 2026** (Download via `externalexam.bsnl.co.in`).
* **Vacancies**: 100 total (**26 seats in Karnataka/Bangalore**!).
* **Website**: `externalexam.bsnl.co.in`

### 7. Coal India Limited (CIL) -- Management Trainee (Systems)
* **Status**: **Application Submitted on 11 June 2026**.
* **Next Event**: CBT Exam Date Notice Pending on `coalindia.in` (Expected Late August / September 2026). Selection 100% CBT-only.
* **Website**: `coalindia.in`

### National Fertilizers Limited (NFL) -- Management Trainee (IT)
* **Advt No.**: `01 (NFL)/2026`
* **CBT Date**: **11 July 2026 (Completed)**
* **Scorecard Status**: **SCORECARD RELEASED!**
  * **Discipline**: `69.00 / 100`
  * **Aptitude**: `30.00 / 50`
  * **Total Score**: **`99.00 / 150` (66.00%)** (No negative marking)
* **Vacancy & Category**: **1 Post (OBC-NCL Backlog)**
* **Shortlisting Ratio for Interview**: **1 : 5** (Top 5 candidates called for 1 post).
* **Final Weightage**: **80% CBT Score + 20% Personal Interview** (Min 50% qualifying in both).
* **Interview Likelihood**: **VERY HIGH** to be in Top 5 shortlist!
* **Saved Scorecard**: `C:\Programs\Learning\Gov jobs\NFL\ScoreCard_NFL Recruitment Portal.pdf`
* **Website**: `careers.nfl.co.in`

### 9. Balmer Lawrie -- Executive Trainee (IT)
* **Status**: **CBT Completed on 15 July 2026**.
* **Next Event**: Awaiting CBT Result publication.
* **Website**: `balmerlawrie.com/careers`

---

## 📝 Category II: Immediate Applications To Fill (ACTIVE)

These are immediate high-priority applications that Avanish must fill now:

**📊 CANONICAL TABLE:** For the full structured list with pay / gross / in-hand / selection process / OBC-NCL reservation / CS eligibility per form, see **`.brain/Jobs-Table.md`** (5 tiers, 19 forms, last updated 20 Aug 2026).

### 1. HAL (Hindustan Aeronautics Limited) -- Design Trainee (DT) Computer Science
* **Advt No.**: `HAL/CHRC-TM/RECT-02/2026` (Dated 29.07.2026)
* **Status**: **FORM SUBMITTED (13 Aug 2026)** — App No: `D321947`
* **Saved PDF Copy**: `HAL/Detailed_Advertisement_29072026.md`
* **Dedicated Workspace Guide**: `HAL/README.md`
* **OFFICIAL CBT EXAM DATE**: **05 & 06 September 2026** 🔥 (*Verified from Official Advt PDF!*)
* **Official Selection Timeline**:
  - **Admit Card Download**: 20 August 2026
  - **CBT Exam**: 05 & 06 September 2026 (150 Mins, 160 MCQs: 20 GA + 40 Eng/Reasoning + 100 CS)
  - **CBT Results Declaration**: 10 September 2026
  - **Interview Call Letter**: 12 September 2026
  - **Interviews at Bengaluru**: 21 to 25 September 2026
  - **Induction Training at Bengaluru**: 29 October 2026
* **Location & Domain**: R&D Centres (predominantly **Bengaluru & Tumakuru**!). Avionics, flight control software, radar/electronic warfare, embedded systems, RTOS, defence AI/ML.
* **Selection Weightage**: 85% CBT Marks + 15% Personal Interview (Min 50% qualifying in both).
* **Fee**: ₹500 (inclusive of GST) via SBIePAY.
* **Website**: `hal-india.co.in`

### 2. RCFL (Rashtriya Chemicals and Fertilizers Limited) -- Management Trainee (Information Technology)
* **Advt No.**: `16022026` / `MTIT/022026`
* **Status**: **OPENS AUGUST 8, 2026 (Closes August 24, 2026)** 🟢
* **Vacancies**: 5 (UR: 2, OBC-NCL: 2, SC: 1).
* **Eligibility**: B.E. / B.Tech in CSE / IT / Computer Engineering (Min 60% aggregate, 55% for SC).
* **Age Limit**: 27 years for UR, 30 years for OBC-NCL (as on 1.2.2026).
* **Stipend & Grade**: ₹60,000/month during training, absorbed in E-1 Grade (₹40,000 – 1,40,000).
* **Selection**: Online CBT + Personal Interview.
* **Website**: `rcfltd.com`

### 3. NMDC Steel Limited (NSL) -- Executive Trainee (ET) -- Computer & Information Technology
* **Advt No.**: `Notification No. 05 of 2026`
* **Status**: **REGISTRATION SUBMITTED -- PAYMENT PENDING**
* **Applicant Details**:
  - Name: AVANISH KUMAR GUPTA
  - Registration No.: **126595**
  - Phone: 8107432468
  - Email: avanish.gupta.official1@gmail.com
  - Fee: ₹500.00 (Paid on portal, awaiting gateway confirmation)
  - Applied Discipline: EXECUTIVE TRAINEE - COMPUTER & INFORMATION TECHNOLOGY
* **Payment Note**: Click "Pay Now" on portal, select category `RECRUITMENT NOTIFICATION NO. 05/2026 NSL`. After payment, wait 48-72 hours for bank validation before downloading application form.
* **Location & Pay**: Nagarnar, Chhattisgarh. Pay Scale: ₹50,000 - 1,60,000.
* **Selection**: CBT + Group Discussion (GD) + Interview.
* **Last Date**: **11 August 2026** ✅ Form submitted; CBT/GD/Interview TBD
* **Website**: `nmdcsteel.nmdc.co.in`
* **Folder**: `NMDCSteel/`

### 4. Airports Authority of India (AAI) -- Junior Executive (Operations) / Manager
* **Advt No.**: `Advt. No. 12/2026/CHQ/DR-CBT`
* **Status**: **OPENS AUGUST 8, 2026** — verify apply window live on aai.aero 🟢
* **Eligibility & Requirement**: CS/IT Degree eligible for JE (Operations). Requires LMV driving license for airside duty vehicles. Fee: ₹0 for SC/ST/PwBD/Women.
* **Selection**: CBT + Interview / Verification.
* **Website**: `aai.aero`

### 5. CERT-In (Indian Computer Emergency Response Team) -- Scientist 'B'
* **Advt No.**: `CERT-In/SCB/2026/1`
* **Status**: **🔥 FORM SUBMITTED SUCCESSFULLY! (17 August 2026)** 🟢
* **Application No**: **`261030011869`**
* **GATE Credentials**: GATE 2026 CS (Score: `350`) + GATE 2026 DA (Score: `337`)
* **Saved Confirmation**: `C:\Programs\Learning\Gov jobs\ICERT\ConfirmationPage-261030011869.pdf`
* **Vacancies**: **83 Posts for CS / IT** (Pay Level-10: ₹56,100 - 1,77,500, Group 'A' / Statutory Body under MeitY).
* **Selection Weightage**: 60% GATE Score + 40% Personal Interaction/Interview (at New Delhi).
* **Website**: `cert-in.org.in` / `examinationservices.nic.in`

### 6. ISRO -- Scientist/Engineer 'SC' (Computer Science)
* **Advt No.**: `ISRO:ICRB:02(EMC-CEPO):2026`
* **Status**: **🔥 FORM SUBMITTED (17 August 2026)** 🟢
* **GATE Credentials**: GATE 2026 CS (Reg: `CS26S31313020`, Score: `350`, Rank: `29099`)
* **Saved Scorecard Upload**: `GATE_2026_CS_Scorecard_Avanish.pdf` (358 KB)
* **Selection**: GATE 2026 CS Score Shortlisting + Personal Interview (URSC Bangalore).
* **Website**: `isro.gov.in`

### 7. ISRO ICRB 01(A-JPA):2026 -- Assistants / JPA / UDC / Stenographer (267 posts) ⚠️ CLOSES TODAY
* **Advt No.**: `ISRO:ICRB:01(A-JPA):2026`
* **Status**: **🔥 APPLY TODAY! Closes 20 Aug 2026 (2 days left)**
* **Vacancies**: **267** (vacancy increased Aug 2026)
* **Eligibility**: Graduate in any discipline (CS qualifies)
* **Pay**: Level-4 ₹25,500 / Level-6 ₹35,400 (7th CPC)
* **Selection**: CBT (qualifying) + Skill Test (Steno typing) + DV
* **Website**: `isro.gov.in/CurrentOpportunities.html`

### 8. SBI Junior Associate (Clerk) -- OBC-NCL Backlog Special Drive 🟢 APPLY THIS WEEK
* **Advt No.**: `CRPD/CR/SPLDRIVE/2026-27/16`
* **Status**: **Closes 27 August 2026**
* **Eligibility**: Graduate in any discipline; age 20-28 + 3 OBC-NCL relaxation
* **Pay**: Clerk scale ~₹26,730–₹32,050 + DA + allowances
* **Selection**: Prelims → Mains → LGD
* **Advantage**: **OBC-NCL backlog quota = lower competition**
* **Website**: `sbi.bank.in/web/careers`

### 9. SBI Specialist Cadre Officer (3 active notifications) ⚠️ VERIFY POST NAMES FIRST
* **Advt No.**: `CRPD/SCO/2026-27/11` (Contract/Regular), `12` (Regular), `13` (Contract)
* **Status**: **Closes 27 August 2026**
* **Eligibility**: Varies by post; previous cycle had Asst Manager (System) + Dy Manager (Systems-InfoSec)
* **Pay**: MMGS-II to III (~₹36,000–₹63,920 basic)
* **Selection**: Online Test (Quant + Reasoning + Eng + Prof Knowledge) → Interview/GD
* **Action**: Open PDFs on sbi.bank.in/web/careers and search for IT/Systems posts
* **Website**: `sbi.bank.in/web/careers`

### 10. IOCL Executives Recruitment 2026 (470 posts) ⚠️ VERIFY CS ELIGIBILITY
* **Advt No.**: `IOCL/CO-HR/RECTT/2026/01`
* **Status**: **Closes 03 September 2026** (apply window: 14 Aug – 3 Sep 17:00 hrs)
* **Vacancies**: **470** (Engineer/Officer/AQCO/JE across Grade A/A0/E0)
* **Eligibility**: Grade A (0-3 yrs): ELIGIBLE; A0 (2-5 yrs): BORDERLINE (~3.08 yrs); E0: INELIGIBLE
* **Pay**: Grade A ₹50,000–₹1,60,000; A0 ₹60,000–₹1,80,000; E0 ₹70,000–₹2,00,000
* **Selection**: Online CBT (Domain + General Aptitude) → GD → Interview → Medical
* **Action**: Verify CS eligibility in detailed advt PDF on iocl.com/latest-job-opening
* **Website**: `iocl.com/latest-job-opening`

### 11. ISRO HSFC:01:RMT/2026 -- Scientist/Engineer 'SD' ⚠️ VERIFY EXP
* **Advt No.**: `HSFC:01:RMT/2026`
* **Status**: **Closes 30 August 2026**
* **Vacancies**: SD posts (Bengaluru posting at HSFC)
* **Eligibility**: CS/IT B.E./B.Tech + post-qualification experience (verify 3-yr floor)
* **Pay**: Level-11 ₹67,700–₹2,08,700 (7th CPC)
* **Selection**: CBT + Interview
* **Action**: Verify 3-yr experience clause (your ~3.08 yrs likely qualifies)
* **Website**: `isro.gov.in/CurrentOpportunities.html`

### 12. RCFL Management Trainee 2026 -- 94 Posts (NEW, 10 Aug 2026 notification)
* **Advt No.**: RCFL MT Recruitment 2026 (verify on rcfltd.com/hrrecruitment)
* **Status**: **Verify exact last date — likely late Aug 2026**
* **Vacancies**: **94** (multi-discipline MT)
* **Eligibility**: B.E./B.Tech CS/IT likely eligible; verify CS discipline count
* **Pay**: ₹40,000–₹1,40,000 (E-1, Navratna PSU)
* **Selection**: CBT + GD + Interview
* **Website**: `rcfltd.com/hrrecruitment`

### 13. URSC:03:2026 -- ISRO URSC Graduate Apprentice (Bengaluru)
* **Advt No.**: `URSC:03:2026`
* **Status**: **Closes 28 August 2026**
* **Eligibility**: B.Tech CS/IT graduate
* **Pay**: Stipend ~₹9,000–11,000/mo (1-year apprenticeship)
* **Selection**: CGPA-based shortlisting + DV
* **Website**: `isro.gov.in/CurrentOpportunities.html`

### 14. HARTRON (Haryana State Electronics Dev Corp) -- Junior Programmer
* **Advt No.**: `Hartron/ICTET/2026-27/03`
* **Status**: ❌ **CLOSED 09 August 2026** (missed window)
* **Vacancies**: **530 Posts** (State PSU / Corp).
* **Eligibility**: B.Tech / MCA / B.Sc / BCA in CS/IT (Min 55% marks).
* **Selection**: Written Test / Programming Test.
* **Website**: `hartron.org.in`

---

## 🛑 Category V: Completed, Closed, Dead & Missed Tracks

These are tracks where selection cycles have concluded, or missed due to deadline lapses:

### 1. Cotton Corporation of India Ltd. (CCI) -- Direct Recruitment 2026
* **Advt No.**: `DR/CCI/2026`
* **Status**: **MISSED / CLOSED** 🔴 (Lapsed due to missed deadline).

### 2. Armoured Vehicles Nigam Limited (AVNL) -- Fixed Term Contract 2026
* **Status**: **MISSED / CLOSED** 🔴 (Fixed Term Contract, lapsed).

### 3. SEBI IT Officer -- Grade A
* **Status**: **Closed / No Interview Call**. Did not qualify in the Phase 2 coding round.
* **Website**: `sebi.gov.in`

### 4. ECIL MT
* **Status**: **Closed / Not Shortlisted**. Did not clear the CBT cutoff score.
* **Website**: `ecil.co.in`

### 6. HURL (Hindustan Urvarak & Rasayan Limited) -- Advt E/04/2026
* **Advt No.**: `E/04/2026` (Dated 05 August 2026)
* **Application Dates**: 05 August 2026 to 26 August 2026
* **Status**: **AUDITED / NOT ELIGIBLE** 🔴
* **Reason**: IT posts (Senior Engineer SAP) require **minimum 4 years of post-qualification executive experience**. Avanish has ~1.1 years of full-time experience (2024 passout).
* **Website**: `jobs.hurl.net.in`

---

## 🌟 Category III: Newly Identified & Emerging Opportunities

These are highly competitive CS/IT roles currently accepting applications that Avanish should actively target.

### 1. BSNL JTO (Junior Telecom Officer) Telecom -- Open Market Direct Recruitment 2026
* **Why it's a gold mine**: Direct recruitment (not through GATE) for 100 vacancies, with **26 seats in Karnataka (Bangalore)**!
* **Application Dates**: **04 June 2026 to 03 July 2026 (10:00 AM)**.
* **CBT Exam Date (Tentative)**: **August 2026**.
* **Vacancies**: 100 (Gen/OBC/EWS: ₹2000 fee, SC/ST: ₹1000). Karnataka has 26 seats, Kerala has 15, Maharashtra has 16, MP has 10.
* **Pattern**: 120 Questions (480 Marks) in 3 sections (Engg Section I: 50 Qs, Engg Section II: 50 Qs, General Ability: 20 Qs).
* **Website**: `externalexam.bsnl.co.in`

### 2. EXIM Bank -- Deputy Manager (IT)
* **Last Date**: **14 June 2026**.
* **Vacancies**: 4.
* **Website**: `eximbankindia.in/careers`

### 3. RSSB Rajasthan Joint Direct Computer Instructor Recruitment 2026
* **Last Date**: **23 June 2026 (11:59 PM)**.
* **Vacancies**: **3,951 total** (3,629 Basic Instructor, 322 Senior Instructor).
* **Exam Dates**: 22 August 2026 (Senior), 23 August 2026 (Basic).
* **Website**: `rssb.rajasthan.gov.in`

### 4. THSTI -- System Administrator (IT)
* **Last Date**: **15 June 2026**.
* **Vacancies**: 1 (Contractual, ₹75,000/month). Needs 2 years experience (fits STMicro + Berkadia perfectly).
* **Website**: `thsti.res.in/careers`

---

## 🔍 Category IV: Parallel Tracks & Ad-Hoc Opportunities (To Check regularly)

We are actively scraping and checking the careers portals for these organizations for upcoming IT Officer/Engineer/GET notifications:

### 1. IFFCO (GET IT)
* **CBT Dates**: Awaiting schedule. Preliminary CBT on own device, final CBT controlled.
* **Check Portal**: `iffco.in/careers`

### 2. KRIBHCO (Krishak Bharati Cooperative Limited)
* **Check Portal**: `kribhco.net/careers`

### 3. Other Major PSUs (To check weekly):
* **BSNL SET (Telecom)**: Written CBT scheduled for **July 5, 2026**. Admit card expected late June. (`externalexam.bsnl.co.in`)
* **UPPSC Polytechnic Lecturer (CS)**: Written exam scheduled for **September 6, 2026**. (`uppsc.up.nic.in`)
* **DRDO Scientist B**: Awaiting GATE-2026 based application notifications.
* **ISRO Scientist SC**: Check `isro.gov.in/careers` for direct/CBT notifications.
* **ONGC, IOCL, GAIL, NTPC, PGCIL, BARC**: Monitor individual career pages weekly for computer science direct recruitments.

---

## 📰 Scraping and Sourcing Guidelines (For Cursor AI)
1. Use `https://www.indgovtjobs.in/` as the primary aggregator. Focus on:
   - "PSU Jobs" category page
   - "Engineer Govt Jobs" category page
   - "Bank Jobs" / "Specialist Officer" listings
2. Conduct regular queries like: `"site:iffco.in careers"`, `"site:kribhco.net recruitment"`, and `"indgovtjobs.in PSU computer science IT"`.
3. If new IT/CS, JTO, SO, Assistant Manager, or Systems MT notifications are found, immediately update this file and present them to Avanish.
