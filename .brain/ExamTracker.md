# Exam Tracker

## Active / Upcoming Exams

### HPCL -- Officer (Information Systems)
| Field | Details |
|-------|---------|
| **Date** | 3 May 2026 |
| **Status** | Applied -- CBT date confirmed on official HPCL job openings page |
| **Priority** | HIGH -- CRITICAL PATH |
| **Pattern** | CBT -> Group Task -> Personal Interview -> Skill Test |
| **Posts** | 20 Officer (IS) posts |
| **Eligibility** | B.E./B.Tech CS/IT, 65% (55% SC/ST/PwBD), age 25 + OBC relaxation |
| **Syllabus** | Official IS syllabus covers Digital Logic, COA, Programming Concepts, DSA, OS, DBMS, CN, Information Security / Cyber Laws, and Emerging Technologies |
| **Bond** | 3-year service bond |
| **Fee** | Rs 1,180 (OBC) |
| **Prep Focus** | Finish the remaining breadth quickly, then shift to mixed mocks, weak areas, and exam execution |

### IFFCO -- Officer / Senior Officer (IT)
| Field | Details |
|-------|---------|
| **Date** | Application open till 28 Apr 2026; treat late May 2026 as the current planning window until a test date is confirmed |
| **Status** | Application live |
| **Priority** | HIGH -- SECONDARY |
| **Pattern** | Online test or shortlisting based on qualification/experience, followed by personal interview and medical exam |
| **Posts** | 7 |
| **Qualification** | Bachelor's degree in IT / Computer Science / Artificial Intelligence |
| **Eligibility** | General/OBC minimum 60%; SC/ST minimum 55%; age 21-27 as on 1 Apr 2026 with OBC relaxation |
| **Experience** | Maximum up to 5 years post-qualification |
| **Role Focus** | Oracle database, SQL, PL/SQL, database administration, Entity Framework Core, C, C#, Bootstrap, Node.js, React, and application-server concepts |
| **Location** | New Delhi / NCR (04), Aonla Unit (02), Paradeep Unit (01) |
| **Prep Focus** | Reuse shared CS core where possible and close the enterprise-stack gaps that are not covered by GATE-style prep |

### UPPSC Polytechnic Lecturer -- Computer Science
| Field | Details |
|-------|---------|
| **Date** | 4 Jun 2026 |
| **Status** | Applied |
| **Priority** | HIGH |
| **Pattern** | 2 papers, 2.5 hrs each. Each paper: 25 Q (Hindi/GS) + 100 Q (CS) = 125 Q, 375 marks. Total: 250 Q, 750 marks, +3/-1 |
| **Posts** | 513 posts |
| **Age Limit** | 21-40 (as of 1 Jul 2025) |
| **Syllabus** | Paper 1: Computer Organization, DSA, Algorithms, DBMS, Networks, Programming, Cyber Security. Paper 2: OS, AI/ML, Compiler Design, DSP, Communication Networks. Both: General Hindi + General Studies |
| **Fee** | Rs 225 (OBC) |
| **Prep Focus** | CS core (shared with HPCL prep) + General Hindi + General Studies |

### BSNL SET -- Senior Executive Trainee (Telecom)
| Field | Details |
|-------|---------|
| **Date** | POSTPONED (was 29 Mar 2026, new date TBD). Registration extended to 31 Mar 2026 |
| **Status** | Applied -- Exam Postponed |
| **Priority** | MEDIUM (date unknown) |
| **Pattern** | CBT: 200 MCQs, 200 marks, 3 hrs, -0.25 per wrong. Part I: 40 Q (Aptitude), Part II: 160 Q (Technical) |
| **Posts** | 95 Telecom + 25 Finance = 120 posts |
| **Age Limit** | 33 (OBC), as of 7 Mar 2026 |
| **Syllabus** | Telecom, Wireless, IP/Data Networks, Wireline, Transmission, IoT/Blockchain/Cloud/AI, Project Management + Aptitude |
| **Fee** | Rs 2,500 (OBC) |
| **Prep Focus** | CS core (shared) + Telecom-specific sprint when date announced |

## Awaiting Result / Interview Update

### MSTC MT -- Management Trainee
| Field | Details |
|-------|---------|
| **Date** | 18 Mar 2026 (GD round completed) |
| **Status** | GD completed; awaiting result / interview update |
| **Priority** | HIGH -- KEEP WARM |
| **Pattern** | ~~CBT~~ -> ~~GD~~ -> Interview |
| **Posts** | 37 vacancies |
| **Prep Focus** | Build interview readiness: HR questions, career-story answers, MSTC fit, and behavioral responses |

## Other Active Applications

### Bank of Baroda -- IT Officer / Specialist Officer
| Field | Details |
|-------|---------|
| **Date** | TBD |
| **Status** | Applied (1-year experience posting) |
| **Priority** | MEDIUM |
| **Pattern** | Online test -> Psychometric -> GD/Interview |
| **Posts** | 418 posts total (Officer Scale I needs 1 year exp) |
| **Salary** | Rs 48,480 - 85,920 |
| **Syllabus** | Full Stack (Java/MERN), Cloud, AI, CS/IT fundamentals |
| **Prep Focus** | CS core + industry experience (strong match given Berkadia work) |

### STPI -- Software Technology Parks of India
| Field | Details |
|-------|---------|
| **Date** | TBD (applied by 12 Jan 2026) |
| **Status** | Applied |
| **Priority** | MEDIUM |
| **Pattern** | Written exam (70%) + Interview (30%) for Scientist B |
| **Posts** | 24 posts (5 Scientist B, rest MTSS) |
| **Age Limit** | 33 (30+3 OBC) for Scientist B |
| **Syllabus** | CS/IT core for Scientist B |
| **Fee** | Rs 1,000 (Group A) |

### HLL Lifecare -- Management Trainee
| Field | Details |
|-------|---------|
| **Date** | TBD |
| **Status** | Applied |
| **Priority** | MEDIUM |
| **Pattern** | CBT -> Preliminary Interview -> Psychometric -> Final Interview |
| **Posts** | 40 posts (17 Engineering + 1 Data Analytics) |
| **Age Limit** | 30 (27+3 OBC), as of 1 Feb 2026 |
| **Stipend** | Rs 40,000/month during training |
| **Fee** | Rs 1,180 (OBC) |

### CEL -- Central Electronics Limited (GET)
| Field | Details |
|-------|---------|
| **Date** | TBD |
| **Status** | Applied |
| **Priority** | LOW (only 1 CSE post) |
| **Pattern** | Shortlisting -> Written test -> PI -> DV |
| **Posts** | 34 total (1 GET Computer Science) |
| **Age Limit** | 30 (27+3 OBC), as of 31 Jan 2026 |
| **Eligibility** | B.E./B.Tech CSE, 55% |

## Awaiting Results

### SEBI IT Officer -- Grade A
| Field | Details |
|-------|---------|
| **Date** | Phase 1: 10 Jan 2026 (done), Phase 2: 21 Feb 2026 (done) |
| **Status** | AWAITING Phase 2 result |
| **Pattern** | Phase 1: Paper 1 (GA/English/QA/Reasoning, 100 marks, 60 min) + Paper 2 (IT MCQs, 100 marks, 40 min). Phase 2: Paper 1 (English Descriptive, 100 marks, 60 min) + Paper 2 (IT Coding, 100 marks, 180 min) |
| **Posts** | 135 posts |
| **Cutoff (OBC 2024)** | Phase 2: 81, Final: 80.55 |
| **Selection** | ~~Phase 1~~ -> ~~Phase 2~~ -> Interview (15%) |

### ECIL MT -- Electronics Corporation of India Limited
| Field | Details |
|-------|---------|
| **Status** | AWAITING CBT result |
| **Pattern** | CBT (done) |

## Completed / Failed

| Exam | Status | Notes |
|------|--------|-------|
| BARC OCES | CBT Done 14 Mar 2026 -- Attempted ~60/100 | Struggled: SQL, CN, TOC, OS(deadlock), COA, Algo. C prog partial. Awaiting result. |
| BEL MT | FAILED CBT | Need to improve CS core fundamentals |
| BDL MT | FAILED CBT | OBC cutoff was 112 marks. Core CS weakness. |
| CBSE Asst. Secretary | FAILED | Could not clear cutoff |
| GATE 2026 CS | Score: 21 | Not usable for PSU shortlists |
| GATE 2026 DA | Score: 28 | Not usable for PSU shortlists |

## Priority Queue (Ordered by Date)

1. **HPCL Officer (IS)** -- 3 May 2026 CBT
2. **IFFCO Officer / Senior Officer (IT)** -- application closes 28 Apr 2026; plan as a late-May target until the test date is confirmed
3. **MSTC MT** -- awaiting GD result / interview update
4. **UPPSC Lecturer** -- 4 Jun 2026
5. **BSNL SET** -- TBD (postponed)
6. **Bank of Baroda / STPI / HLL / CEL** -- TBD
7. **SEBI IT / ECIL** -- waiting for results
