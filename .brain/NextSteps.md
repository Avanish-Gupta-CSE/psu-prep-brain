# Next Steps & Handoff (PSU Exam Preparation Reboot & Sprints)

Last updated: August 20, 2026 (Thursday, 08:05 IST) — Active Form-Filling Day

This is the canonical resume-point file for any future login, agent, or chat system.
Read this file first to understand the current priority queue and the immediate day-by-day action plan.
For a structured table of every open form with pay / process / CS eligibility, read `.brain/Jobs-Table.md`.

---

## 🚨 IMMEDIATE PRIORITY: 19 Forms to Fill — Sync with `.brain/Jobs-Table.md`

**Today is the LAST DAY for ISRO ICRB 267 (closes 20 Aug).** All other active forms close 24 Aug – 3 Sep.

The canonical action list with pay / gross / in-hand / selection process / OBC-NCL reservation / CS eligibility is in:
**→ `.brain/Jobs-Table.md` (Tier 1–5, 19 forms)**

### ⚡ TODAY (20 Aug 2026, before ISP CBT prep)
1. **ISRO ICRB 267 (Assistant/JPA/UDC/Steno)** — `.brain/Jobs-Table.md` row #1, closes today
   Apply: https://www.isro.gov.in/CurrentOpportunities.html
2. **Open SBI SCO 11/12/13 PDFs** at https://sbi.bank.in/web/careers (search for IT/Systems posts; **Bing extraction returned only site nav, must read the PDF directly**)

### 📋 THIS WEEK (by 27 Aug 2026)
3. SBI JA OBC-NCL Backlog Drive — Jobs-Table row #2
4. SBI SCO 11/12/13 — Jobs-Table rows #3-5 (verify IT posts first)
5. SBI JA General — Jobs-Table row #10

### 🎯 21–27 Aug 2026 — Exam Sprint Window
6. **RCFL MTIT (5 posts, 2 OBC reserved, closes 24 Aug)** — Jobs-Table row #6 — closes **4 days after today**
7. **RCFL MT 94-post (Aug 10 notification, verify CS discipline)** — Jobs-Table row #7
8. **URSC Apprentice (Bengaluru, closes 28 Aug)** — Jobs-Table row #8 — resume line

### 📅 30 Aug – 3 Sep 2026
9. ~~**ISRO HSFC:01:RMT/2026 (Scientist/Engineer SD)**~~ — **NOT ELIGIBLE** — Jobs-Table row #12. Skip; needs 3+ yrs post-qualification exp, you have ~2.2 yrs.
11. **IOCL Executives Advt 2026/01 (470 posts)** — Jobs-Table row #9 — verify Grade A CS eligibility in PDF
12. **AAI Advt 12/2026/CHQ/DR-CBT (JE Operations)** — Jobs-Table row #13 — only if you have LMV license

---

## 📅 Active Exam Timeline (Aug-Sep 2026)

| Date | Exam | Location | Status |
|---|---|---|---|
| **24 Aug 2026 (Mon)** | **Coal India Limited (CIL) MT (Systems)** | Bangalore | Applied (43 Seats, CBT-Only) — **PRIMARY CS TECHNICAL TARGET** 🔥 |
| **23 Aug 2026 (Sun)** | **HLL Lifecare MT (CS/IT)** | Lucknow | Admit card `117272`, flights booked |
| **23 Aug 2026 (Sun)** | **BSNL JTO** (100 posts, 26 Bengaluru seats) | Bangalore | Admit card drops TODAY (20 Aug) |
| **05–06 Sep 2026 (Sat/Sun)** | **HAL Design Trainee (CS)** | Bangalore | Submitted (App `D321947`) — admit card drops 20 Aug |
| **06 Sep 2026 (Sat)** | **UPPSC Polytechnic Lecturer (CS)** | UP | Paper-I + Paper-II |

---

## 🆕 Forms Already Submitted (Tier 4 in Jobs-Table.md)

| Form | App No | Submitted |
|---|---|---|
| ISRO SC (CS) — GATE-based | GATE CS 350 | 17 Aug 2026 |
| CERT-In Scientist 'B' (MeitY) | `261030011869` | 17 Aug 2026 |
| HAL DT (CS) — Bengaluru R&D | `D321947` | 13 Aug 2026 |
| NHAI DM (IT) — GATE-based | `2026772` | 05 Aug 2026 |
| NMDC Steel ET (CS/IT) | `126595` (Fee ₹500) | 11 Aug 2026 |

---

## 📊 Administrative & Handoff Status

- **Berkadia LWD:** 31 Aug 2026 (early release approved); active handovers for ReservesAI, DMS, CMS
- **UCO Bank SO Onboarding:** September 2026 batch (Salt Lake, Kolkata); extension granted
- **GATE 2026 Scores:** CS 350 (AIR 29,099), DA 337
- **Experience:** ~2.2 yrs post-qualification (Berkadia Jun 2024 – present; STMicro internship Aug 2023 – May 2024 was during B.Tech so NOT counted). IOCL Grade A eligible; Grade A0 just clears 2-yr floor; HSFC SD (3+ yrs) INELIGIBLE.
- **Age:** 25 → 26 on 31 Mar 2026; OBC-NCL relaxation applies for most PSU posts through 30+

---

## 🚫 NOT Eligible / Closed

- **HURL Advt E/04/2026** — needs 4 yrs post-qualification exp; you have ~2.2 yrs → **INELIGIBLE**
- **HARTRON Jr Programmer** — closed 9 Aug 2026 (missed)
- **MSTC MT** — not selected (Jul 2026 merit)
- **STPI Scientist 'B'** — not selected (68.80/100)

---

## 📁 Brain File Index

- `.brain/NextSteps.md` — **this file** (resume-point, dated 20 Aug 2026)
- `.brain/Progress.md` — session log + status header
- `.brain/ExamTracker.md` — exams sorted by date (next: ISP 19 Aug done, CIL 24 Aug)
- `.brain/TrackedJobs.md` — staged/completed tracks (needs sync with Jobs-Table.md Tier 4)
- `.brain/Jobs-Table.md` — **CANONICAL FORM TABLE** (5 tiers, 19 forms, pay/process/eligibility)
- `.brain/StudyPlan.md` — phase schedule (stale on remote; needs Phase 5/6 update for Aug-Sept sprint)
- `.brain/Profile.md` — student profile (exp calc corrected to ~2.2 yrs post-qualification)
- `.brain/WeakAreas.md` — weak topic tracking
- `.brain/Syllabus.md` — GATE CS syllabus
- `.brain/QuickReference.md` — formula cheat sheets
- `.brain/MockTestLog.md` — mock scores
- `.brain/Motivation.md` — motivational notes
- `.brain/README.md` — directory overview

---

## 🛠️ Stale Files (need update next session)

1. `.brain/StudyPlan.md` — still shows Phase 4 as ACTIVE with HPCL focus (HPCL is closed/dead); needs Phase 5/6 for Aug-Sept CBT marathon
2. ~~`.brain/Profile.md` — `Experience` field still says "~1.1 yrs"; update to ~3.08 yrs~~ ✅ DONE — corrected to ~2.2 yrs post-qualification
3. `AGENTS.md` — priority rules still say "until HPCL CBT on 3 May 2026" (past); needs Phase-5/6 priorities
4. `.brain/TrackedJobs.md` — needs Tier-5 sync with Jobs-Table.md (add ISRO ICRB 267, SBI SCO, RCFL MT 94-post, IOCL 470, HSFC SD, NABARD Grade A)
5. `.brain/ExamTracker.md` — needs to add the new forms being filled (Tier 1-3 of Jobs-Table)