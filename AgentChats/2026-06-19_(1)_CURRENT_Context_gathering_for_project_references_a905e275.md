# (1) CURRENT: Context gathering for project references

| Metadata | Value |
| :--- | :--- |
| **Chat ID** | `a905e275-4a87-4f86-8b7e-96817c3d6c19` |
| **Created At** | `2026-06-19 19:11:05` |
| **Last Updated At** | `2026-06-19 19:11:05` |
| **Files Touched / Summary** | Edited Avanish Gupta Achievements.md |
| **Model** | `gpt-5.2` |
...
```mermaid
graph TD
    subgraph digitalChannels ["Digital Channels Layer"]
        ucoMBanking["UCO mBanking Plus"]
        internetBanking["Internet Banking"]
        upiApps["UPI Apps / ATMs"]
    end

    subgraph securityLayer ["Security & Middleware Layer"]
        apiGateway["API GATEWAY LAYER (Rate Limiting, Keycloak SSO, WAF, SSL Termination)"]
    end

    subgraph cbsLayer ["Core Banking (CBS) Layer"]
        finacleCore["FINACLE CORE ENGINE (General Ledger, Customer Master (CIF), Transaction Engine)"]
    end

    subgraph dataLayer ["Data Storage Layer"]
        centralDb["CENTRAL TRANSACTION DATABASE (Highly Normalized RDBMS, ACID Compliant)"]
    end

    ucoMBanking -->|"HTTPS / REST"| apiGateway
    internetBanking -->|"HTTPS / REST"| apiGateway
    upiApps -->|"HTTPS / REST"| apiGateway
    apiGateway -->|"ISO 8583 / ISO 20022"| finacleCore
    finacleCore -->|"SQL / ACID"| centralDb
```
...
```mermaid
graph TD
    step1["Step 1: Start Transaction (Avanish transfers ₹10,000 to B)"]
    step2["Step 2: Consistency Check (Is Balance >= ₹10,000?)"]
    step3["Step 3: Isolation Lock (Acquire S/X locks on A and B)"]
    step4["Step 4: Atomicity Execution (Debit A, Credit B)"]
    step5["Step 5: Durability Log (Write to WAL on disk)"]
    step6["Step 6: Commit / Rollback (Permanently save or undo)"]
    rollbackNode["ROLLBACK PATH (Undo changes via WAL log, restore original balances)"]

    step1 --> step2
    step2 -->|"Yes (Pass)"| step3
    step3 --> step4
    step4 -->|"Success"| step5
    step5 -->|"Commit"| step6

    step2 -->|"No (Fail)"| rollbackNode
    step4 -->|"Crash (Fail)"| rollbackNode
```
...
```mermaid
graph TD
    subgraph collectionsHierarchy ["Collections Hierarchy"]
        colInt["Collection (Interface)"]
        listInt["List (Interface)"]
        setInt["Set (Interface)"]
        arrList["ArrayList (Fast Reads)"]
        lnkList["LinkedList (Fast Inserts)"]
        hshSet["HashSet (Unique Items)"]
        mapInt["Map (Interface)"]
        hshMap["HashMap (Key-Value)"]
        conHshMap["ConcurrentHashMap (Thread-Safe Cache)"]
    end
    ...
    subgraph multithreading ["Multithreading & Concurrency"]
        threadPool["Thread Pool / ExecutorService (Manages worker threads)"]
        thread1["Thread 1 (Process Tx A)"]
        thread2["Thread 2 (Process Tx B)"]
        thread3["Thread 3 (Process Tx C)"]
        sharedResource["Shared Resource (Account Balance) [Synchronized / Locks / Atomic]"]
    end

    threadPool --> thread1
    threadPool --> thread2
    threadPool --> thread3
    thread1 -->|"Acquires lock"| sharedResource
    thread2 -->|"Blocked / Wait"| sharedResource
    thread3 -->|"Acquires lock"| sharedResource
```
